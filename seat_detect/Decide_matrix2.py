#from seat_detect import Histogram2 as h
'''
def seat_size(mhist_w, mhist_h):
    # 좌석의 가로 길이 판단하기
    seat_w = -100
    min_seat_w = -100
    max_seat_w = -100
    for i in range(len(mhist_w)):
        if(mhist_w[i]!=0):
            #min_seat가 최초값이라면
            if (min_seat_w < 0):
                min_seat_w = i
        ...
        #해당 좌표의 픽셀 개수가 0이라면-즉, 빈 공간이라면
        else:
            #하지만 min_seat_w가 초기값이 아니면 - 좌석 하나만큼 훑었다는 뜻
            if(min_seat_w >0):
                break

    seat_w = max_seat_w - min_seat_w

    # 좌석의 세로 길이 판단하기
    seat_h = -100
    min_seat_h = -100
    max_seat_h = -100
    for i in range(len(mhist_h)):
        if (mhist_h[i] != 0):
            # min_seat가 최초값이라면
            if (min_seat_h < 0):
                min_seat_h = i
        ...
        # 해당 좌표의 픽셀 개수가 0이라면
            ...
            # 하지만 min_seat_h가 초기값이 아니면 - 좌석 하나만큼 훑었다는 뜻
            if (min_seat_h > 0):
                break

    seat_h = max_seat_h - min_seat_h

    return seat_w, seat_h, min_seat_w
'''
...
# 줄 별로 행렬 만들기
'''
def define_matrix(mhist_w, min_x, seat_w):
    w = len(mhist_w)
    matrix = [0] * (int((w-min_x) / seat_w))
...
'''
seat_w, seat_h, min_x = seat_size(h.mhist_w, h.mhist_h)
lines = seat_line(h.mhist_h)
print(lines)
print(seat_h)
min_y, max_y = set_roi(lines, seat_h)
print(min_y, max_y)
matrix = define_matrix(h.mhist_h, min_x, seat_w)
'''